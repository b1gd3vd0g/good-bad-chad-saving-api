...
if (!env.PLAYER_MICROSERVICE) {
    require('dotenv').config({
        path: '../../.env'
    });
}
const { PLAYER_MICROSERVICE } = env;

// Error out if process.env actually isn't configured.
if (!PLAYER_MICROSERVICE) {
    throw new Error(
...
const fetchPlayerByToken = async (token) => {
    try {
        const response = await fetch(`${PLAYER_MICROSERVICE}/`, {
            method: 'GET',
            headers: {
        ...
        switch (response.status) {
            case 200:
                ...
                return cr(200, player);
            case 401:
                return cr(401, 'Player not found.');
            default:
                return cr(500, 'Internal server error.');
        }
    } catch (e) {
        return cr(500, e);
    }
